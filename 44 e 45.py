import os
import subprocess

"""Exercício 44 e 45
44) Crie um algoritmo que leia o valor inicial da contagem, o valor final e o
incremento, mostrando em seguida todos os valores no intervalo: Ex: Digite o
primeiro Valor: 3
Digite o último Valor: 10
Digite o incremento: 2
Contagem: 3 5 7 9 Acabou!
45) O programa acima vai ter um problema quando digitarmos o primeiro valor
maior que o último. Resolva esse problema com um código que funcione em qualquer
situação.
"""

# ============================================================
#  LIMPAR ECRÃ (Windows / Linux / macOS)
# ============================================================
def cls():
    """Limpa o terminal de forma compatível com vários sistemas operativos."""
    comando = "cls" if os.name == "nt" else "clear"
    subprocess.run(comando, shell=True)

from colorama import Fore, Style, init
init(autoreset=True)
import time

while True:
    cls()
    print (Fore.CYAN + Style.BRIGHT + "\n🔢 Simulador de Contagem Personalizada\n")

    # Introdução dos valores
    ninicio = int(input(Fore.YELLOW + "✏️  Digite o valor inicial da contagem: "))
    nfim = int(input(Fore.YELLOW + "✏️  Digite o valor final da contagem: "))
    incremento = int(input(Fore.YELLOW + "✏️  Digite o incremento: "))

    # Introdução dos valores
    if incremento == 0:
        print(
            Fore.RED
            + Style.BRIGHT
            + "\n⚠️  O incremento não pode ser zero. Tente novamente."
        )
        time.sleep(4)
        continue

    # Validação: incremento incompatível com direção da contagem
    if (ninicio < nfim and incremento < 0) or (ninicio > nfim and incremento > 0):
        print(
            Fore.RED
            + Style.BRIGHT
            + "\n⚠️  O incremento não é compatível com os valores inicial e final. Tente novamente."
        )
        time.sleep(4)
        continue

    # Resumo da configuração
    print(Fore.CYAN + "\n📘 Resumo da configuração:")
    print(f"🟢 Início: {ninicio}")
    print(f"🟡 Fim: {nfim}")
    print(f"🔵 Incremento: {incremento}\n")
    time.sleep(2)

    # Preparação da contagem
    print(Fore.GREEN + Style.BRIGHT + "⏳ A iniciar contagem...\n")
    time.sleep(2)

    # Caso especial: início igual ao fim
    if ninicio == nfim:
        print(Fore.GREEN + Style.BRIGHT + "\nContagem:", end=" ")
        print(ninicio, end=" ")

    # Ajuste automático do incremento conforme direção
    if ninicio < nfim:
        incremento = abs(incremento)
    else:
        incremento = -abs(incremento)

    # Cabeçalho da contagem
    if (ninicio < nfim and incremento > 0) or (ninicio > nfim and incremento < 0):
        print(Fore.GREEN + Style.BRIGHT + "\nContagem:", end=" ")

    # Execução da contagem
    for i in range(ninicio, nfim + 1, incremento):
        print(i, end=" ", flush=True)
        time.sleep(0.5)
    print(Fore.GREEN + Style.BRIGHT + "\nAcabou! ✔️")

    # Pergunta para nova simulação
    novo = (
        input(Fore.MAGENTA + "\n🔁 Deseja simular novamente? (s/n): ").strip().lower()
    )

    if novo != "s":
        cls()
        print(Fore.CYAN + Style.BRIGHT + "\n👋 Obrigado por simular! Até a próxima!\n")
        break
